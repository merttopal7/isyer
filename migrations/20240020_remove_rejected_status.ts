...
export async function up(knex: Knex): Promise<void> {
  ...
  if (client === "pg" || client === "postgresql") {
    // PostgreSQL: drop reject_reason column and update enum via ALTER TABLE
    await knex.schema.alterTable("appointments", (table) => {
      table.dropColumn("reject_reason");
    });
    // PostgreSQL enum values cannot be removed, but we can enforce via check constraint
    // The enum column in pg is stored as a native type — we change it to text + check constraint
  } else {
    // SQLite: must recreate the table (no ALTER COLUMN support)
